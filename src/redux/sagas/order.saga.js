...
function* updateSampleLab(action) {
  try {
    ...
    if (
      action.payload.delayed !== response.data.delayed &&
      response.data.delayed === true
    ) {
      ...
      yield put({
        ...
        payload: {
          ...
          message:
            'Unfortunately there was an issue with your sample and it has been delayed. Somebody should be in contact with you shortly with more information. ',
        },
      }); // end dispatch
    }
